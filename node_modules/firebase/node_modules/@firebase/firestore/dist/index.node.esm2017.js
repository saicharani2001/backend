...
const version = "7.22.1-alpha.96ae6a5fe";
        ...
        if (!this.db) {
            ...
            this.db = await new Promise((resolve, reject) => {
                ...
                request.onupgradeneeded = (event) => {
                    logDebug(LOG_TAG$2, 'Database "' + this.name + '" requires upgrade from version:', event.oldVersion);
                    const db = event.target.result;
                    this.schemaConverter
                        .createOrUpgrade(db, request.transaction, event.oldVersion, this.version)
                        .next(() => {
                        logDebug(LOG_TAG$2, 'Database upgrade to version ' + this.version + ' complete');
                    });
                };
            });
        }
    ...
    async runTransaction(action, mode, objectStores, transactionFn) {
        ...
        while (true) {
            ...
            try {
                ...
                logDebug(LOG_TAG$2, 'Transaction failed with error:', error.message, 'Retrying:', retryable);
                this.close();
                if (!retryable) {
...
class BundleLoader {
     ...
     */
    async complete() {
        const changedDocuments = await applyBundleDocuments(this.localStore, this.documents, this.bundleMetadata.id);
 ...
 * queue.
 */
async function applyBundleDocuments(localStore, documents, bundleName) {
    const localStoreImpl = debugCast(localStore);
    ...
    const umbrellaTargetData = await allocateTarget(localStoreImpl, umbrellaTarget(bundleName));
    return localStoreImpl.persistence.runTransaction('Apply bundle documents', 'readwrite', txn => {
        return populateDocumentChangeBuffer(txn, documentBuffer, documentMap, SnapshotVersion.min(), versionMap)
            ...
            return localStoreImpl.targetCache
                ...
                .next(() => localStoreImpl.localDocuments.getLocalViewOfDocuments(txn, changedDocs));
        });
    });
}
/**
...
function hasNewerBundle(localStore, bundleMetadata) {
    const localStoreImpl = debugCast(localStore);
    const bundleConverter = new BundleConverter(localStoreImpl.serializer);
    const currentReadTime = bundleConverter.toSnapshotVersion(bundleMetadata.createTime);
    return localStoreImpl.persistence
 ...
 */
async function saveNamedQuery(localStore, query, documents = documentKeySet()) {
    // Allocate a target for the named query such that it can be resumed
    // from associated read time if users use it to listen.
    // NOTE: this also means if no corresponding target exists, the new target
    // will remain active and will not get collected, unless users happen to
    // unlisten the query somehow.
    const allocated = await allocateTarget(localStore, queryToTarget(fromBundledQuery(query.bundledQuery)));
    const localStoreImpl = debugCast(localStore);
 ...
 * limitations under the License.
 */
const LOG_TAG$8 = 'OnlineStateTracker';
...
async function onWatchStreamChange(remoteStoreImpl, watchChange, snapshotVersion) {
    ...
    if (watchChange instanceof WatchTargetChange &&
        ...
        try {
            ...
            logDebug(LOG_TAG$9, 'Failed to remove targets %s: %s ', watchChange.targetIds.join(','), e);
    ...
    if (!snapshotVersion.isEqual(SnapshotVersion.min())) {
        try {
            ...
            logDebug(LOG_TAG$9, 'Failed to raise snapshot:', e);
            await disableNetworkUntilRecovery(remoteStoreImpl, e);
        }
    }
}
...
class AsyncQueue {
    ...
    async retryNextOp() {
        ...
        try {
            ...
            if (isIndexedDbTransactionError(e)) {
                logDebug(LOG_TAG$a, 'Operation failed with retryable error: ' + e);
            }
            else {
...
async function loadBundleImpl(syncEngine, reader, task) {
    try {
        ...
        // has newer version.
        await emitNewSnapsAndNotifyLocalStore(syncEngine, result.changedDocs, 
...
class IndexedDbOfflineComponentProvider extends MemoryOfflineComponentProvider {
    ...
    createPersistence(cfg) {
        ...
        return new IndexedDbPersistence(cfg.persistenceSettings.synchronizeTabs, persistenceKey, cfg.clientId, LruParams.withCacheSize(cfg.persistenceSettings.cacheSizeBytes), cfg.asyncQueue, getWindow(), getDocument(), this.serializer, this.sharedClientState, cfg.persistenceSettings.forceOwningTab);
    }
    createSharedClientState(cfg) {
        return new MemorySharedClientState();
    }
...
class BundleReader {
    ...
    static fromBundleSource(source, serializer) {
...
class FirestoreClient {
    ...
    start(databaseInfo, offlineComponentProvider, onlineComponentProvider, persistenceSettings) {
        ...
        this.credentials.setChangeListener(user => {
            if (!initialized) {
                ...
                this.asyncQueue.enqueueRetryable(() => remoteStoreHandleCredentialChange(this.remoteStore, user));
            }
        });
    ...
    waitForPendingWrites() {
        this.verifyNotTerminated();
        const deferred = new Deferred();
        this.asyncQueue.enqueueAndForget(() => registerPendingWritesCallback(this.syncEngine, deferred));
        return deferred.promise;
    }
    listen(query, options, observer) {
        ...
        this.asyncQueue.enqueueAndForget(() => eventManagerListen(this.eventMgr, listener));
        return () => {
            wrappedObserver.mute();
            this.asyncQueue.enqueueAndForget(() => eventManagerUnlisten(this.eventMgr, listener));
        };
    }
    async getDocumentFromLocalCache(docKey) {
        ...
        const deferred = new Deferred();
        this.asyncQueue.enqueueAndForget(() => readDocumentFromCache(this.localStore, docKey, deferred));
        return deferred.promise;
    }
    async getDocumentViaSnapshotListener(key, options = {}) {
        ...
        await this.initializationDone.promise;
        const deferred = new Deferred();
        this.asyncQueue.enqueueAndForget(() => readDocumentViaSnapshotListener(this.eventMgr, this.asyncQueue, key, options, deferred));
        return deferred.promise;
    }
    async getDocumentsFromLocalCache(query) {
        ...
        await this.initializationDone.promise;
        const deferred = new Deferred();
        this.asyncQueue.enqueueAndForget(() => executeQueryFromCache(this.localStore, query, deferred));
        return deferred.promise;
    }
    async getDocumentsViaSnapshotListener(query, options = {}) {
        ...
        await this.initializationDone.promise;
        const deferred = new Deferred();
        this.asyncQueue.enqueueAndForget(() => executeQueryViaSnapshotListener(this.eventMgr, this.asyncQueue, query, options, deferred));
        return deferred.promise;
    }
    write(mutations) {
        this.verifyNotTerminated();
        const deferred = new Deferred();
        this.asyncQueue.enqueueAndForget(() => syncEngineWrite(this.syncEngine, mutations, deferred));
        return deferred.promise;
    }
    databaseId() {
    ...
    addSnapshotsInSyncListener(observer) {
        this.verifyNotTerminated();
        const wrappedObserver = new AsyncObserver(observer);
        this.asyncQueue.enqueueAndForget(async () => addSnapshotsInSyncListener(this.eventMgr, wrappedObserver));
        return () => {
            wrappedObserver.mute();
            this.asyncQueue.enqueueAndForget(async () => removeSnapshotsInSyncListener(this.eventMgr, wrappedObserver));
        };
    }
    ...
    loadBundle(data, resultTask) {
        ...
        this.asyncQueue.enqueueAndForget(async () => {
            ...
            return resultTask.catch(e => {
                logWarn(LOG_TAG$e, `Loading bundle failed with ${e}`);
            });
        });
    }
 ...
 */
class FieldPath$1 extends _BaseFieldPath {
    /**
...
class UserDataWriter {
    constructor(databaseId, timestampsInSnapshots, serverTimestampBehavior, referenceFactory, bytesFactory) {
        this.databaseId = databaseId;
 ...
 */
class DocumentReference extends _DocumentKeyReference {
...
class DocumentSnapshot {
    ...
    data(options) {
        ...
        if (!this._document) {
            ...
            if (this._converter) {
                ...
                const userDataWriter = new UserDataWriter(this._firestore._databaseId, this._firestore._areTimestampsInSnapshotsEnabled(), options.serverTimestamps || 'none', key => new DocumentReference(key, this._firestore, /* converter= */ null), bytes => new Blob(bytes));
    ...
    get(fieldPath, options) {
        ...
        if (this._document) {
            ...
            if (value !== null) {
                const userDataWriter = new UserDataWriter(this._firestore._databaseId, this._firestore._areTimestampsInSnapshotsEnabled(), options.serverTimestamps || 'none', key => new DocumentReference(key, this._firestore, this._converter), bytes => new Blob(bytes));
...
const version$1 = "1.18.0-alpha.96ae6a5fe";
