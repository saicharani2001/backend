...
class UploadTask {
     ...
     */
    constructor(ref, service, location, mappings, blob, metadata = null) {
        this.transferred_ = 0;
    ...
    createResumable_() {
        this.resolveToken_(authToken => {
            const requestInfo = createResumableUpload(this.service_, this.location_, this.mappings_, this.blob_, this.metadata_);
    ...
    fetchStatus_() {
        ...
        this.resolveToken_(authToken => {
            const requestInfo = getResumableUploadStatus(this.service_, this.location_, url, this.blob_);
    ...
    continueUpload_() {
        ...
        this.resolveToken_(authToken => {
            let requestInfo;
            try {
                requestInfo = continueResumableUpload(this.location_, this.service_, url, this.blob_, chunkSize, this.mappings_, status, this.makeProgressCallback_());
            }
            catch (e) {
                this.error_ = e;
                this.transition_(InternalTaskState.ERROR);
    ...
    fetchMetadata_() {
        this.resolveToken_(authToken => {
            const requestInfo = getMetadata(this.service_, this.location_, this.mappings_);
    ...
    oneShotUpload_() {
        this.resolveToken_(authToken => {
            const requestInfo = multipartUpload(this.service_, this.location_, this.mappings_, this.blob_, this.metadata_);
            const multipartRequest = this.service_.makeRequest(requestInfo, authToken);
...
class Reference {
    ...
    delete() {
        ...
        return this.service.getAuthToken().then(authToken => {
            const requestInfo = deleteObject(this.service, this.location);
    ...
    list(options) {
        ...
        return this.service.getAuthToken().then(authToken => {
            const op = options || {};
            const requestInfo = list(self.service, self.location, 
    ...
    getMetadata() {
        ...
        return this.service.getAuthToken().then(authToken => {
            const requestInfo = getMetadata(this.service, this.location, this.mappings());
            return this.service.makeRequest(requestInfo, authToken).getPromise();
        });
    }
    /**
    ...
    updateMetadata(metadata) {
        ...
        return this.service.getAuthToken().then(authToken => {
            const requestInfo = updateMetadata(this.service, this.location, metadata, this.mappings());
            return this.service.makeRequest(requestInfo, authToken).getPromise();
        });
    }
    /**
    ...
    getDownloadURL() {
        ...
        return this.service.getAuthToken().then(authToken => {
            const requestInfo = getDownloadUrl(this.service, this.location, this.mappings());
            return this.service
...
class StorageService {
    ...
    makeRequest(requestInfo, authToken) {
        if (!this.deleted_) {
            const request = makeRequest(requestInfo, this.appId_, authToken, this.pool_);
            this.requests_.add(request);
            // Request removes itself from set when complete.
            request.getPromise().then(() => this.requests_.delete(request), () => this.requests_.delete(request));
            return request;
        }
        else {
...
const version = "0.3.43-alpha.96ae6a5fe";
